...
export const _getAllProducts = async ({ eventId, isEnabled = true }) => {
  try {
    const {
...
export const _getAllPhotos = async ({ eventId, nextToken }) => {
  try {
    const {
      data: {
        getAllPhotos: { photos, nextToken: nt },
      },
    } = await API.graphql({
      query: getAllPhotos,
      variables: {
    ...
    return {
      photos: photos ? photos.filter((p) => p.isEnabled) : [],
      nextToken: nt,
    };
  } catch (error) {
    console.log(error);
...
export const _getSchedule = async ({ eventId }) => {
  try {
