...
export default function BarApp() {
  ...
  async function getAllOrdersByStatus({ nT, eventId }) {
    let nextToken = nT;

    let params = nextToken ? { eventId, nextToken } : { eventId };
    try {
      let { users: fetchedUsers, nextToken: next } = await _getAllOrdersByStatus(params);

      if (fetchedUsers) dbOrders = dbOrders.concat(fetchedUsers);

      if (next) {
        await getAllOrdersByStatus({ nT: nextToken, eventId });
      } else {
