  ...
  return (
    <Grid item xs={4} spacing={1} container direction="column" style={{ overflowY: "auto" }}>
      {Object.keys(groupedByTable).map((o) => {
        return <GridCell key={o} table={groupedByTable[o]} color={type === ORDER_STATUS.INLINE ? COLORS.redDark : type === ORDER_STATUS.CHOOSING ? COLORS.yellowDark: COLORS.greenDark}} />;
      })}
    </Grid>
  );
}

function GridCell({ table }) {
...
export default function BarApp() {
  ...
  async function getAllOrdersByStatus({ nT, eventId, status }) {
    ...
    try {
      ...
      if (fetchedOrders) setBarOrders([...state.barOrders, ...fetchedOrders]); // dbOrders = dbOrders.concat(fetchedOrders);
  ...
  console.log("jhsdfbksjnfkjsdnfvskjn", state.barOrders.length);
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <Grid
          ...
          style={{ marginLeft: "auto", marginRight: "auto" }}
        >
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.INLINE]} type={ORDER_STATUS.INLINE} />
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.CHOOSING]} type={ORDER_STATUS.CHOOSING} />
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.IN_PROGRESS]} type={ORDER_STATUS.IN_PROGRESS} />
        </Grid>
      </Grid>
    </div>
  );
