...
function GridRow({ order }) {
  ...
  const [time, setTimer] = useState({
    inline: inline ? inline.date : Date.now(),
    choosing: choosing ? choosing.date : Date.now(),
    inprogress: inprogress ? inprogress.date : Date.now(),
  });

  const height =
    Math.max(
      inline ? inline.products.length : 0,
      Math.max(choosing ? choosing.products.length : 0, inprogress ? inprogress.products.length : 0)
    ) * 40;

  useEffect(() => {
    const interval = setInterval(() => {
      setTimer({
        inline: Date.now() - time.inline,
        choosing: Date.now() - time.choosing,
        inprogress: Date.now() - time.inprogress,
      });
    }, 1000);
    ...
    <React.Fragment>
      {inline && (
        <Grid item xs={4}>
          <Paper
            ...
            style={{ height, overflow: "auto", border: `0.5px solid ${COLORS.redLight}` }}
          >
      ...
      {choosing && (
        <Grid item xs={4}>
          <Paper
            className={classes.paper}
            style={{ height, overflow: "auto", border: `0.5px solid ${COLORS.yellowLight}` }}
          >
            <Typography variant="body1" style={{ fontWeight: "bold" }}>
      ...
      {inprogress && (
        <Grid item xs={4}>
          <Paper
            className={classes.paper}
            style={{ height, overflow: "auto", border: `0.5px solid ${COLORS.greenDark}` }}
          >
            <Typography variant="body1" style={{ fontWeight: "bold" }}>
