...
const contextReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "set_fetch_orders":
      ...
      break;
    case "toggle_update_db_order":
      newState = { ...state, updateDBOrder: action.payload };
      break;
    case "toggle_update_local_order":
      newState = { ...state, updateLocalOrder: action.payload };
      break;
    default:
      newState = state;
      break;
  ...
  localStorage.setItem(
    ...
    JSON.stringify({ event: newState.event, user: newState.user, table: newState.table })
  );
