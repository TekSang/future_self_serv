...
function GridRow({ order }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTimer({
        inline: new Date(Date.now() - time.inline).toISOString().substr(11, 8),
        choosing: new Date(Date.now() - time.choosing).toISOString().substr(11, 8),
        inprogress: new Date(Date.now() - time.inprogress).toISOString().substr(11, 8),
      });
    }, 1000);
