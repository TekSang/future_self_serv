...
export default function BarApp() {
  ...
  async function getAllTableIds({ nT, eventId, status }) {
    let nextToken = nT;

    let params = nextToken ? { eventId, status, nextToken } : { eventId, status };
    try {
      let { tables: fetchedIds, nextToken: next } = await _getAllTableIds(params);

      if (fetchedIds) dbTableIds = dbTableIds.concat(fetchedIds);

      if (next) {
        await getAllTableIds({ nT: nextToken, eventId, status });
      } else {
