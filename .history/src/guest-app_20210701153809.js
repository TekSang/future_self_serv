...
import { COLORS, PAGES, defaultState, ORDER_STATUS, EnableStatus } from "./libs/constants";
...
import { _updateProduct } from "./graphql_operations/mutations";
  ...
  useEffect(() => {
    if (products.length > 0) {
      for (var i = 0; i < products.length; i++) {
        ...
        _updateProduct({ eventId: event.id, id: product.id, isEnabled: EnableStatus.TRUE });
      }
      let p = getAllProducts({ eventId: event.id });
      console.log(p);
    }
  }, [products]);
