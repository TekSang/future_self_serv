...
import { _getEventByCode, _getAllUsersAtEvent, _getUser } from "./graphql_operations/queries";
...
function BarApp(props) {
  ...
  async function getUser({ id }) {
    try {
      let dbUser = await _getUser({ id });
      return dbUser;
    } catch (error) {
      console.log("🚀 ~ file: guest-app.js ~ line 199 ~ getUser ~ error", error);
    }
  }
