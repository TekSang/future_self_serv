  ...
  function GridRow({ order, index }) {
    ...
    return (
      <React.Fragment>
        {inline && (
          <Grid item xs={4}>
            <Paper className={classes.paper} style={{ border: `0.5px solid ${COLORS.redLight}` }}>
              <Typography variant="h6">{inline.table}</Typography>
              <List dense={true}>
                {inline.products.map((item) => {
                  return (
                    <ListItem>
                      <ListItemText primary={item.name} />
                      <ListItemText style={{ textAlign: "center" }} primary={item.quantity} />
                    </ListItem>
                  );
                })}
              </List>
        ...
        {choosing && (
          <Grid item xs={4}>
            <Paper className={classes.paper} style={{ border: `0.5px solid ${COLORS.yellowLight}` }}>
              {choosing.table}
            </Paper>
          </Grid>
        )}
        {inprogress && (
          <Grid item xs={4}>
            <Paper className={classes.paper} style={{ border: `0.5px solid ${COLORS.greenDark}` }}>
              {inprogress.table}
            </Paper>
          </Grid>
        )}
