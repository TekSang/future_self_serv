...
export default function BarApp() {
  ...
  useEffect(() => {
    dbOrders = [];
    let inlineOrders = getAllOrdersByStatus({ eventId: state.event.id, status: ORDER_STATUS.INLINE });
    console.log("🚀 ~ file: bar-app.js ~ line 129 ~ useEffect ~ inlineOrders", inlineOrders);
    dbOrders = [];
    let choosingOrders = getAllOrdersByStatus({ eventId: state.event.id, status: ORDER_STATUS.CHOOSING });
    console.log("🚀 ~ file: bar-app.js ~ line 132 ~ useEffect ~ choosingOrders", choosingOrders);
    dbOrders = [];
    let inprogressOrders = getAllOrdersByStatus({ eventId: state.event.id, status: ORDER_STATUS.IN_PROGRESS });
    console.log("🚀 ~ file: bar-app.js ~ line 135 ~ useEffect ~ inprogressOrders", inprogressOrders);
    return () => {
      console.log("cleanup");
    };
  }, [subscribeEvent]);
