...
export default function BarApp() {
  ...
  async function getAllOrdersByStatus({ nT, eventId, status }) {
    ...
    try {
      ...
      if (newFetchedOrders && status === ORDER_STATUS.INLINE) dbInlineOrders = dbInlineOrders.concat(newFetchedOrders);

      if (next) {
        await getAllOrdersByStatus({ nT: nextToken, eventId, status });
      } else {
        if (status === ORDER_STATUS.INLINE) setInlineOrders(dbInlineOrders);
        return;
      }
