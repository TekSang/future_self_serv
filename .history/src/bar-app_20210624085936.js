...
export default function BarApp() {
  ...
  useEffect(() => {
    ...
    for (const i in statuses) {
      getAllOrdersByStatus({ eventId: state.event.id, status: statuses[i], buffer: dbOrders });
  ...
  async function getAllOrdersByStatus({ nT, eventId, status, buffer }) {
    let nextToken = nT;
    ...
    try {
      ...
      if (fetchedOrders) buffer = buffer.concat(fetchedOrders);
    ...
    <div className={classes.root}>
      ...
      >
        ...
        <Grid
          ...
          style={{ marginLeft: "auto", marginRight: "auto" }}
        >
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.INLINE]} />
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.CHOOSING]} />
          <Column orders={getSortedOrders(state.barOrders)[ORDER_STATUS.IN_PROGRESS]} />
        </Grid>
      </Grid>
    </div>
  );
