...
import { _getEventByCode, _getAllUsersAtEvent, _getUser as getUser, _getOrder } from "./graphql_operations/queries";
        ...
        if (user) {
          ...
          if (dbUsers.map((item) => item.user.id.toLowerCase()).includes(user.id.toLowerCase())) {
            ...
            if (dbUser) {
              ...
              console.log("🚀 ~ file: guest-app.js ~ line 171 ~ onLoad ~ userState", userState);
              //let timeSinceLastLogin = Date.now()  - new Date(userState.lastLogin).getTime()
