...
export default function BarApp() {
  ...
  useEffect(() => {
    let r = _.groupBy(allOrders, function (order) {
      ...
      >
        ...
        >
          <Grid item xs={4} spacing={1} container direction="column">
            {sortedOrders[ORDER_STATUS.INLINE].map((o) => {
              return <GridCell key={Math.random()} />;
            })}
          </Grid>
          <Grid item xs={4} spacing={1} container direction="column">
            {sortedOrders[ORDER_STATUS.CHOOSING].map((o) => {
              return <GridCell key={Math.random()} />;
            })}
          </Grid>
          <Grid item xs={4} spacing={1} container direction="column">
            {sortedOrders[ORDER_STATUS.IN_PROGRESS].map((o) => {
              return <GridCell key={Math.random()} />;
            })}
          </Grid>
        </Grid>
