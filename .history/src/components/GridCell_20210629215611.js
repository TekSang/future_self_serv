...
export default function GridCell({ table, status }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (status === ORDER_STATUS.CHOOSING) setInactivityCounter(inactivityCounter + 1);
    }, 1000);
  ...
  if (status === ORDER_STATUS.CHOOSING) {
    console.log(table[0].table.name, inactivityCounter);
  }
