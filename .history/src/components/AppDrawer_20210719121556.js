...
function AppDrawer(props) {
  ...
  const isDayOfEvent = Date.now() >= new Date(new Date(event.dateOfEvent).toDateString()).getTime();
  ...
  console.log(order.status !== ORDER_STATUS.IN_PROGRESS && isDayOfEvent);
        ...
        >
          <div>
            ...
            <List>
              ...
              {order.status !== ORDER_STATUS.IN_PROGRESS || isDayOfEvent ? (
                <ListItem
                  button
                  onClick={() => {
                    if (mobileOpen) {
