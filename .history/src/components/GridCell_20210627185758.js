...
export default function GridCell({ table, status }) {
  ...
  function restructureProducts(orders) {
    ...
    let uniqueProductIds = _.uniq(allProducts.map((p) => p.id));

    allProducts.forEach((element) => {
      let product = restructured.filter((p) => uniqueProductIds.includes(element.id));
      if (product) {
        ...
        restructured = [...restructured, product];
      }
    });
