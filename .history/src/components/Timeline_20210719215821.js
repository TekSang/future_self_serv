      ...
      >
        {schedule.map((item, i) => {
          return (
            <ListItem
              key={i}
              style={{ margin: 5, width: "100%", backgroundColor: i % 2 === 0 ? COLORS.greenLight : COLORS.orange }}
            >
              <Typography variant="inherit" style={{ width: 75, fontSize: 16, fontWeight: "bold" }}>
                {moment(item.time).format("LT")}
              </Typography>

              <ListItemText style={{ textAlign: "left" }}>
                <Typography variant="inherit" style={{ fontSize: 14, fontWeight: "bold" }}>
