...
import { getOrdersByStatus } from "../graphql/queries";
...
const completedThreshold = 3; //180000; // milliseconds, 3 mins

export default function GridCell({ table, status }) {
  const classes = useStyles();
  const initTime = new Date(_.min(table.map((t) => t.order.created))).getTime();
  const completedInitTime = new Date(_.min(table.map((t) => t.order.modified))).getTime();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if ([ORDER_STATUS.IN_PROGRESS, ORDER_ACTIONS.INLINE].includes(getOrdersByStatus)) {
        setCompletedCounter(Date.now() - completedInitTime);
      }
    }, 1000);
