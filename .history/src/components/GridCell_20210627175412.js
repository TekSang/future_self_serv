...
export default function GridCell({ table, status }) {
  ...
  async function getOrdersOnTable() {
    try {
      let dbOrders = await _getOrdersOnTable({
        tableId: table[0].table.id,
        statuses: [status],
        date: new Date(Date.now() - 1000 * 60 * 60 * 24).toJSON(),
      });
      return dbOrders;
    } catch (error) {
      console.log("🚀 ~ file: GridCell.js ~ line 98 ~ getOrdersOnTable ~ error", error);
    }
  }
  ...
  useEffect(() => {
    getOrdersOnTable()
      .then(({ orders }) => {
        setProducts(
          _.uniqBy(_.flatten(orders.map((o) => o.products)), function (p) {
            return p.product.id;
          })
        );
      })
